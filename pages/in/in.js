...
Page({
  ...
  data: {
    ...
    accountIndex: '0',
    rangeList: ['无', '生活费', '奖学金', '助学金', '兼职收入', '意外收入', '其他收入','理财收入'],
    rangeIndex: '0',
  ...
  formSubmit(e) {
    ...
    } else if (that.data.time == '无') {
      wx.showModal({
        title: '系统提示',
        content: '时间不能为空',
        showCancel: false
      })
    } else if (that.data.accountIndex == 0) {
      wx.showModal({
        title: '系统提示',
        content: '账户不能为空',
        showCancel: false
      })
    } else if (that.data.rangeIndex == 0) {
      wx.showModal({
        title: '系统提示',
        content: '类别不能为空',
        showCancel: false
      })
    } else if (that.data.submitValue.rmb == '') {
      wx.showModal({
        title: '系统提示',
        content: '金额不能为空',
        showCancel: false
